...
<div class="fade-in" style="display: flex; align-items: center; justify-content: center; min-height: 80vh; padding: 20px 0;">
    <div class="studio-card" style="width: min(100%, 450px); padding: 32px; margin: 0; box-shadow: var(--shadow-lg);">
        ...
        <div style="display: flex; background: rgba(0,0,0,0.2); border-radius: 12px; padding: 4px; margin-bottom: 28px;">
            <button id="loginTab" onclick="showAuth('login')" style="flex: 1; height: 40px; border: none; border-radius: 9px; cursor: pointer; font-weight: 700; font-size: 0.85rem; transition: all 0.3s ease; background: var(--primary); color: #000; outline: none;">Sign In</button>
            <button id="registerTab" onclick="showAuth('register')" style="flex: 1; height: 40px; border: none; border-radius: 9px; cursor: pointer; font-weight: 700; font-size: 0.85rem; transition: all 0.3s ease; background: transparent; color: var(--text-dim); outline: none;">Register</button>
        </div>
        ...
        <div id="loginSection">
            ...
            <form method="POST" action="{{ route('login') }}" class="control-group" style="gap: 16px;">
                @csrf
                <label class="control-label" style="font-size: 0.7rem;">Username or Email</label>
                <input type="text" name="login" class="text-input" style="height: 48px;" placeholder="Operator ID or Email" value="{{ old('login') }}" required autofocus>
                @error('login') <p class="drop-subtext" style="color: var(--error); font-size: 0.7rem; margin-top: 2px;">{{ $message }}</p> @enderror
...
@if (($errors->has('name') || $errors->has('password')) && old('name'))
    showAuth('register');
@elseif ($errors->has('login') || $errors->has('password'))
    showAuth('login');
@endif
